...
const Contact: React.FC = () => {
  ...
  const cards = [
    ...
    },
    {
      ...
      icon: 'fa-phone-alt',
      items: [
        { icon: 'fa-phone', label: 'contactPhoneTitle', rawValue: '+244 929 294 029' },
                  ...
                  {card.items?.map((item, idx) => (
                    <li key={idx} className="mb-4 flex items-start gap-4">
                      ...
                      <div>
                        ...
                        {item.rawValue && (
                          item.label === 'contactPhoneTitle' ? (
                            <a href={`tel:${item.rawValue.replace(/[^\d+]/g, '')}`} className="hover:text-accent transition-colors underline decoration-accent/30 underline-offset-4">
                              {item.rawValue}
                            </a>
                          ) : item.label === 'contactEmailTitle' ? (
                            <a href={`mailto:${item.rawValue}`} className="hover:text-accent transition-colors underline decoration-accent/30 underline-offset-4">
                              {item.rawValue}
                            </a>
                          ) : (
