            ...
            {isExpanded && (
                <div className="px-6 pb-6 border-t border-gray/10 pt-4">
                    ...
                    <div className="space-y-4">
                        ...
                        <button 
                            onClick={() => alert(`Configuração para ${integration.name} salva com sucesso!`)}
                            className="bg-accent text-black px-6 py-2 rounded-lg font-semibold hover:brightness-110 transition-all"
                        >
                            Save Configuration
                        </button>
                    </div>
...
export const AdminDashboardContent: React.FC<AdminDashboardContentProps> = ({ onClose, isModal = true }) => {
    ...
    const handleAddVideo = (e: React.FormEvent) => {
        ...
        const videoId = extractYouTubeId(newVideoId);
        
        if (videoId.length !== 11) {
            alert('Por favor, insira um ID de vídeo ou URL do YouTube válido.');
            return;
        }

        if (videoId && newVideoTitle) {
            addVideo({ id: videoId, title: newVideoTitle });
            setNewVideoId('');
            setNewVideoTitle('');
        }
    };
